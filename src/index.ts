...
/**
 * Nebraska Revised Statutes — state statutes by citation.
 *
 * The citation IS the query string: ?statute=28-303. Nothing to derive.
 *
 * SILENT FAILURE: an unknown section returns HTTP 200 with a short page and no
 * statute, so the section heading is confirmed rather than the status trusted.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Nebraska Revised Statutes');
}

const BASE = 'https://nebraskalegislature.gov/laws/statutes.php?statute=';
const UA = 'pipeworx-mcp-nebraska-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'ne_revised_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "28-303" (murder 1) or "60-6,196" (DUI).' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'ne_revised_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '');
  if (!/^\d+-[\d,.-]+$/.test(raw)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Nebraska Revised Statutes citation.`,
      hint: 'Nebraska sections look like "28-303" (murder 1) or "60-6,196" (DUI).' };
  }
  const url = `${BASE}${encodeURIComponent(raw)}`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (!res.ok) throw new Error(`Nebraska Legislature returned HTTP ${res.status} for ${raw}`);
  const html = await readStatuteResponse(res);
  // The statute lives in a .statute block; the page around it is navigation.
  const text = textFromContainer(html, /class="statute"/) ?? statuteText(html);

  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const at = text.search(new RegExp(`${esc}[\\s.]`));
  if (at === -1 || text.length < 2500) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the Nebraska Revised Statutes. The site returns a page for unknown citations, so this is read from the content rather than the status code.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  const body = text.slice(at).trim();
  return {
    citation: `Neb. Rev. Stat. § ${raw}`, section: raw,
    text: body, text_chars: body.length, url,
    source: 'Nebraska Legislature (nebraskalegislature.gov), keyless',
    attribution: 'Nebraska statutes are public record.',
  };
}
